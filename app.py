...
with tab2:
    ...
    if not filtered_data.empty:
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        bars = ax.bar(companies, monetized_emissions_intensity_ratio_values)
...
with tab3:
    ...
    if not filtered_data.empty:
        ...
        ebitda_minus_emissions_values = filtered_data['ebitda_minus_monetized_emissions']

        fig, ax = plt.subplots(figsize=(12, 6))
        x = np.arange(len(companies))
        width = 0.35

        bars1 = ax.bar(x - width/2, ebitda_values, width, label='EBITDA')
        bars2 = ax.bar(x + width/2, ebitda_minus_emissions_values, width, label='EBITDA minus Monetized Emissions')

        ax.set_xlabel('Company')
        ax.set_ylabel('Billions USD')
        ax.set_title('EBITDA vs EBITDA minus Monetized Emissions')
        ax.set_xticks(x)
        ax.set_xticklabels(companies, rotation=45, ha='right')
        ax.legend()
